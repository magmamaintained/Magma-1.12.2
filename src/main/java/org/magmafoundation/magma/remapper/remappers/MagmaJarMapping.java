 ...
 * @author md-5
 * @since 11/11/2019 - 07:32 am
 */
public class MagmaJarMapping implements ClassRemapperSupplier {
    ...
    public final Map<String, ClassMappings> byNMSInternalName = new HashMap<>();
    public final Map<String, ClassMappings> byNMSSrcName = new HashMap<>();
    public final Map<String, ClassMappings> byNMSName = new HashMap<>();
    ...
    private String directFastMapFieldName(boolean inverse, Class clazz, String name) {
        ...
        ClassMappings mapping = byMCPName.get(clazz.getName());
        if (mapping == null) {
            return null;
        }
    ...
    public ClassMappings registerClassMapping(String nmsSrcName, String mcpSrcName) {
        nmsSrcName = nmsSrcName.intern();
        mcpSrcName = mcpSrcName.intern();
        classes.put(nmsSrcName, mcpSrcName);
        ClassMappings mapping = byNMSSrcName.get(nmsSrcName);
        if (mapping == null) {
            mapping = new ClassMappings();
    ...
    public void registerFieldMapping(String nmsSrcName, String nmsName, String mcpSrcName, String mcpName) {
        ...
        ClassMappings mapping = registerClassMapping(nmsSrcName, mcpSrcName);
    ...
    private void parseCsrgLine(String line, MappingTransformer inputTransformer, MappingTransformer outputTransformer, Remapper reverseMap) throws IOException {
        ...
        if (tokens.length == 2) {
            ...
            String oldClassName = inputTransformer.transformClassName(tokens[0]);
            ClassMappings mapping = byNMSSrcName.get(oldClassName);
