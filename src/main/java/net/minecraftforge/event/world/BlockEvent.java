...
public class BlockEvent extends Event
{
    ...
    public static class HarvestDropsEvent extends BlockEvent
    {
        ...
        public HarvestDropsEvent(World world, BlockPos pos, IBlockState state, int fortuneLevel, float dropChance, List<ItemStack> drops, EntityPlayer harvester, boolean isSilkTouching)
        {
            ...
            //Magma start - handle event on bukkit side
            if(harvester == null) return;
            org.bukkit.event.block.BlockBreakEvent bukkitEvent = CraftEventFactory.callBlockBreakEvent(world, pos, state, (EntityPlayerMP) harvester);

            if(bukkitEvent.isCancelled()){
                // Magma start
                if (state.getBlock() instanceof BlockDoublePlant) {
                    BlockPos otherHalfPos = state.getValue(BlockDoublePlant.HALF) == BlockDoublePlant.EnumBlockHalf.UPPER ? pos.down() : pos.up();
                    ((EntityPlayerMP) harvester).connection.sendPacket(new SPacketBlockChange(world, otherHalfPos));
                }
                // Magma end
                this.setCanceled(true);
            } else if(!bukkitEvent.isDropItems()) {
                this.drops.clear();
            }
    ...
    public static class BreakEvent extends BlockEvent
    {
        ...
        public BreakEvent(World world, BlockPos pos, IBlockState state, EntityPlayer player)
        {
            ...
            if(bukkitEvent.isCancelled()){
                ...
                if (state.getBlock() instanceof BlockDoublePlant) {
                    BlockPos otherHalfPos = state.getValue(BlockDoublePlant.HALF) == BlockDoublePlant.EnumBlockHalf.UPPER ? pos.down() : pos.up();
                    ((EntityPlayerMP) player).connection.sendPacket(new SPacketBlockChange(world, otherHalfPos));
                }
                // Magma end
                this.setCanceled(true);
            }
