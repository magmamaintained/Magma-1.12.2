...
public class DimensionManager {
    ...
    public static void setWorld(int id, @Nullable WorldServer world, MinecraftServer server) {
        if (world != null) {
            ...
            if (MagmaConfig.instance.enableAutoUnloadingDimensions.getValues()) { //Magma - Only unload dimensions if auto unloading is enabled
                if (!MagmaConfig.instance.autoUnloadDimensionsWhitelist.contains(id)) { //Magma - Only unload dimensions not in the whitelist
                    try {
                        getWorld(id).saveAllChunks(true, null); //Magma - Save all chunks before unloading
        ...
        for (
                Int2ObjectMap.Entry<WorldServer> entry : worlds.int2ObjectEntrySet()) {
    ...
    private static boolean canUnloadWorld(WorldServer world) {
        return ForgeChunkManager.getPersistentChunksFor(world).isEmpty()
                ...
                && MagmaConfig.instance.enableAutoUnloadingDimensions.getValues()
                && !MagmaConfig.instance.autoUnloadDimensionsWhitelist.contains(world.dimension);
    }
